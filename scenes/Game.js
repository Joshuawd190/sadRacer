...
export default class Game extends Phaser.Scene {
  ...
  create() {
    ...
    this.bg_clouds = this.add
      .tileSprite(0, 0, this.width, this.height, 'clouds')
      .setOrigin(0);
    this.bg_hills = this.add
      .tileSprite(0, 0, this.width, this.height, 'hills')
      .setOrigin(0);
    this.bg_trees = this.add
      .tileSprite(0, 0, this.width, this.height, 'trees')
      .setOrigin(0);

    // particle system for skidding
    ...
    // this.factory = new Phaser.GameObjects.GameObjectFactory(this.scene)
    // this.test = this.factory.polygon()

    //clears the segment array and sets up a new road definition
    //needs a refactor, maybe seperate the reset from the creation and set up arguments for the creation
  ...
  update(time, delta) {
    //Maybe refactor controls into it's own thing that runs here
  ...
  //Add a param to pass in a track config to build or to generate a random one forever
  resetRoad() {
    this.segments = [];
  ...
  addDownhillToEnd(num) {
    ...
    this.addRoad(
      ...
      num,
      0,
      -Math.round(this.lastY() / this.segmentLength)
    );
  }
