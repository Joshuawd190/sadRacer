...
export default class Render {
  ...
  all() {
    let baseSegment = this.scene.findSegment(this.scene.position),
      basePercent = Util.percentRemaining(
        this.scene.position,
        this.scene.segmentLength
      ),
      playerSegment = this.scene.findSegment(
        this.scene.position + this.scene.playerZ
      ),
      playerPercent = Util.percentRemaining(
        this.scene.position + this.scene.playerZ,
        this.scene.segmentLength
      ),
    ...
    this.scene.playerY = Util.interpolate(
    ...
    for (n = 0; n < this.scene.drawDistance; n++) {
      segment =
        this.scene.segments[
          (baseSegment.index + n) % this.scene.segments.length
        ];
      segment.looped = segment.index < baseSegment.index;

      segment.fog = Util.exponentialFog(
        n / this.scene.drawDistance,
      ...
      Util.project(
        segment.p1,
        playerX * roadWidth - x,
        this.scene.playerY + cameraHeight,
  ...
  sprite(sprite, segment) {
    let roadWidth = this.scene.roadWidth,
      ...
      destX =
        segment.p1.screen.x + (scale * sprite.offset * roadWidth * width) / 2,
    ...
    if (clipH < destH) {
      let thisSprite = this.scene.add.image(
        destX,
        destY,
        sprite.source,
        sprite.frame
  ...
  player(segment) {
    let width = this.scene.width,
      ...
      destY =
        height / 2 -
        ((cameraDepth / playerZ) *
          Util.interpolate(y1, y2, playerPercent) *
          height) /
          2,
      steer =
        this.scene.speed *
        (this.scene.keyLeft ? -1 : this.scene.keyRight ? 1 : 0),
      updown = y2 - y1,
      // bounce        = (1.5 * Math.random() * speedPercent * resolution) * Util.randomChoice([-1,1])
      bounceCar =
        1.5 * Math.random() * speedPercent * Util.randomChoice([-1, 1]),
      bounceWheels =
        2.5 * Math.random() * speedPercent * Util.randomChoice([-1, 1]);
    ...
    if (!this.scene.playerCar) {
      this.scene.playerCar = this.scene.add.image(
        destX,
        destY + bounceCar,
        'car_sheet',
        carFrame
    ...
    if (!this.scene.playerWheels) {
      this.scene.playerWheels = this.scene.add.image(
        destX,
        destY + bounceWheels,
        'wheel_sheet',
        carFrame
    ...
    if (!this.scene.playerShadow) {
      this.scene.playerShadow = this.scene.add.image(
        destX,
        shadY,
        'shadow_sheet',
        carFrame
    ...
    this.scene.playerShadow.setAlpha(0.6);
  }
}
